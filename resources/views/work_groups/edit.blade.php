        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-gradient bg-info text-white" style="border-radius: 12px 12px 0 0;">
                    <h4 class="mb-0 fw-semibold">Editar Grupo de Trabajo</h4>
                </div>
                <div class="card-body bg-light">
                    <form action="{{ route('work-groups.update', $workGroup->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label for="name" class="form-label">Nombre del Grupo</label>
                            <input type="text" class="form-control" id="name" name="name" value="{{ $workGroup->name }}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="vehiculo_id" name="vehiculo_id" required>
                                ...
                                @foreach ($vehiculos as $vehiculo)
                                    <option value="{{ $vehiculo->id }}" 
                                        @if ($workGroup->vehiculo_id == $vehiculo->id) selected @endif>
                                    ...
                                    <div class="col-md-6 mb-2">
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="employee_ids[]" value="{{ $employee->id }}"
                                                ...
                                                @if ($workGroup->employees->contains($employee->id)) checked @endif>
                        ...
                        <button type="submit" class="btn btn-success">Actualizar Grupo de Trabajo</button>
